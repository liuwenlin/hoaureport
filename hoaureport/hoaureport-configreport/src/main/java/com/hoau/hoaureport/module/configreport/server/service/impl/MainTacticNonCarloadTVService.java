...
import com.hoau.hoaureport.module.configreport.server.dao.MainTacticNonCarloadTvMapper;
...
@Service
public class MainTacticNonCarloadTVService implements IMainTacticNonCarloadTVService{
	
	private static final Logger log = LoggerFactory.getLogger(MainTacticNonCarloadTVService.class);
	@Resource
	MainTacticNonCarloadTvMapper mapper;
	@Override
	public List<MainTacticNonCarloadTv> queryInfo(MainTacticNonCarloadTv param,
			int start, int limit) {
	...
	@Override
	public Long queryCount(MainTacticNonCarloadTv param) {
		return mapper.queryCountByCondition(param);
	}

	@Transactional
	@Override
	public void addInfo(MainTacticNonCarloadTv info) {
	...
	@Override
	public void updateInfo(MainTacticNonCarloadTv info) {
		UserEntity currUser = (UserEntity) UserContext.getCurrentUser();
		//工号
		String currUserCode = currUser.getEmpCode().substring(2);
		info.setModifyTime(new Date());
	...
	@Override
	public boolean isExist(MainTacticNonCarloadTv param) {
		MainTacticNonCarloadTv exitCondition = new MainTacticNonCarloadTv();
		...
		List<MainTacticNonCarloadTv> SalesLineInfos = queryInfo(exitCondition,0,10);
	...
	public Map<String, Object> bathImplPlatformAreaInfo(String path)
		...
		//导入结果
				...
				//把键值对 转换成 对象集合
				List<MainTacticNonCarloadTv> lines = new ArrayList<MainTacticNonCarloadTv>();
				  if(list.size() > 0){
				...
				for (Map<String, String> map : list) {
					MainTacticNonCarloadTv bean = new MainTacticNonCarloadTv();
					try {
				...
				for (int i = 0; i < lines.size(); i++) {
					MainTacticNonCarloadTv info = lines.get(i);
					try {
						if (info == null) {// 有异常
							continue;
						} else {
	...
	@Override
	public void addOrUpdateInfo(MainTacticNonCarloadTv info,
