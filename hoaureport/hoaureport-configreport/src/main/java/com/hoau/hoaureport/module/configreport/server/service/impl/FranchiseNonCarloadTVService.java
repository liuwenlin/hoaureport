...
import com.hoau.hoaureport.module.configreport.server.dao.FranchiseNonCarloadTvMapper;
...
@Service
public class FranchiseNonCarloadTVService implements IFranchiseNonCarloadTVService{

	private static final Logger log = LoggerFactory.getLogger(FranchiseNonCarloadTVService.class);
	@Resource
	FranchiseNonCarloadTvMapper mapper;
	@Override
	public List<FranchiseNonCarloadTv> queryInfo(FranchiseNonCarloadTv param,
			int start, int limit) {
	...
	@Override
	public Long queryCount(FranchiseNonCarloadTv param) {
		return mapper.queryCountByCondition(param);
	}

	@Transactional
	@Override
	public void addInfo(FranchiseNonCarloadTv info) {
	...
	@Transactional
	@Override
	public void updateInfo(FranchiseNonCarloadTv info) {
		UserEntity currUser = (UserEntity) UserContext.getCurrentUser();
		//工号
		String currUserCode = currUser.getEmpCode().substring(2);
		info.setModifyTime(new Date());
	...
	@Override
	public boolean isExist(FranchiseNonCarloadTv param) {
		FranchiseNonCarloadTv exitCondition = new FranchiseNonCarloadTv();
		...
		List<FranchiseNonCarloadTv> SalesLineInfos = queryInfo(exitCondition,0,10);
	...
	public Map<String, Object> bathImplPlatformAreaInfo(String path)
		...
		//把键值对 转换成 对象集合
		List<FranchiseNonCarloadTv> lines = new ArrayList<FranchiseNonCarloadTv>();
		  if(list.size() > 0){
		...
		for (Map<String, String> map : list) {
			FranchiseNonCarloadTv bean = new FranchiseNonCarloadTv();
			try {
		...
		for (int i = 0; i < lines.size(); i++) {
			FranchiseNonCarloadTv info = lines.get(i);
			try {
				if (info == null) {// 有异常
					continue;
				} else {
	...
	@Override
	public void addOrUpdateInfo(FranchiseNonCarloadTv info,
