/**主界面->部门负责人*/
Ext.define('Ty.Report.WeekDay.WeekDayModel', {
...
/***主界面->次日送达率store*/
Ext.define('Ty.Report.WeekDay.WeekDayStore', {
	extend : 'Ext.data.Store',
	model : 'Ty.Report.WeekDay.WeekDayModel',
	pageSize : 100,// 分页条数
	proxy : {
		type : 'ajax',
		url : 'weekDayAction!queryWeekDay.action',
		actionMethods : 'POST',
		reader : {
			type : 'json',
	...
	listeners : {
		...
		beforeload : function(store, operation, eOpts) {
				var params = {
						'vo.rdmVo.weekdayDate':Ext.String.trim(Ext.getCmp('weekdayDate').getValue()),
...
//			}
			,
			{
				...
				border : true,
				handler:function(){
					weekDayManagerStore.reload();  //加载数据
	...
	//主界面->表格->store
		var weekDayManagerStore=Ext.create('Ty.Report.WeekDay.WeekDayStore',{
			id:'weekDayManagerStoreId'
		});

		/**
		 * 主界面->表格
		 */
		var  weekDayManagerGrid =Ext.create('Ext.grid.Panel', {
			store: weekDayManagerStore,
			border:false,
			columnLines:true,
			...
			{xtype: 'rownumberer',
				width:40},
	        {
	            dataIndex: 'weekdayDate',
				text:'日期',
	            width:150,
	            menuDisabled:true
	        },{
	            dataIndex: 'weekdayDays',
				text:'工作日',
	            width:145,
	            menuDisabled:true
	        }
...
function imple(outFileName,flag){
			if(flag){
				...
				Ext.Ajax.request( {  
			       url : 'weekDayAction!implExcel.action',  
			       method : 'post',  
			       params : {  
	...
	//整个页面布局
		var viewport = Ext.create('Ext.Viewport', {
			...
			items: [queryForm , weekDayManagerGrid]
		});
});
