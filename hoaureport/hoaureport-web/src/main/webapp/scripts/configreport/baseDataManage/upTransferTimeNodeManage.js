...
/**主界面->上转移时间节点模型*/
Ext.define('Ty.Report.UpTransferTimeNode.UpTransferTimeNodeModel', {
...
/***主界面->上转移时间节点store*/
Ext.define('Ty.Report.UpTransferTimeNode.UpTransferTimeNodeStore', {
	extend : 'Ext.data.Store',
	model : 'Ty.Report.UpTransferTimeNode.UpTransferTimeNodeModel',
	pageSize : 100,// 分页条数
	proxy : {
		type : 'ajax',
		url : 'upTransferTimeNodeManageAction!queryUpTransferTimeNode.action',
		actionMethods : 'POST',
		reader : {
			type : 'json',
			rootProperty : 'upTransferTimeNodeVo.upTransferTimeNodeList',
	...
	listeners : {
		...
		beforeload : function(store, operation, eOpts) {
				var params = {
						'upTransferTimeNodeVo.upTransferTimeNode.theArea':Ext.String.trim(Ext.getCmp('theArea').getValue()),
						'upTransferTimeNodeVo.upTransferTimeNode.theRoadArea':Ext.String.trim(Ext.getCmp('theRoadArea').getValue()),
						'upTransferTimeNodeVo.upTransferTimeNode.departureDepartment':Ext.String.trim(Ext.getCmp('departureDepartment').getValue()),
						'upTransferTimeNodeVo.upTransferTimeNode.arrivalDepartment':Ext.String.trim(Ext.getCmp('arrivalDepartment').getValue()),
						'upTransferTimeNodeVo.upTransferTimeNode.active':Ext.getCmp('active').getValue(),
				};
	...
	*/
	 ...
	 var queryForm = Ext.create('Ext.form.Panel',{
			...
			items:[{
				...
				border : true,
				handler:function(){
					upTransferTimeNodeStore.reload();  //加载数据
					//upTransferTimeNodeStore.loadPage(1);	//查询后默认返回第一页
				}
			},{
				xtype:'button',
				text:'重置',
...
//主界面->表格->store
	var upTransferTimeNodeStore=Ext.create('Ty.Report.UpTransferTimeNode.UpTransferTimeNodeStore',{
		id:'upTransferTimeNodeStoreId'
	});

	/**
	 * 主界面->表格
	 */
	var  upTransferTimeNodeGrid =Ext.create('Ext.grid.Panel', {
		store: upTransferTimeNodeStore,
		border:false,
		columnLines:true,
        ...
        listeners : {
		 ...
		 dockedItems:[{//分页
	 			xtype:'pagingtoolbar',
	 			id: 'queryUpTransferTimeNodeGridPaginId',
	 			store:upTransferTimeNodeStore,
	 			dock:'bottom',
	 			displayInfo : true,
	 			autoScroll : true
        ...
        tbar:[{
			...
			handler:function(){
				...
				Ext.getCmp('upOrDownTransferFiled').disable();  
				operateFlag = 2;
				var records = upTransferTimeNodeGrid.getSelectionModel().getSelection();//获取选中集合
				if(records.length == 0){
					showInfoMsg("请选择一条记录！");
					return false;
				};
				...
				var UpTransferTimeNodeModel = new Ty.Report.UpTransferTimeNode.UpTransferTimeNodeModel();
				form.loadRecord(UpTransferTimeNodeModel);
				...
				form.updateRecord(UpTransferTimeNodeModel);
				win.setTitle("数据修改");
				win.show();
			}
		},'-',{//提示 '->' 让按钮居右
			...
			handler:function(){
				...
				confirm("作废后的数据将不再有效，且不能恢复生效，确认作废？",
						function(){
					var formUrl = 'upTransferTimeNodeManageAction!repealUpTransferTimeNode.action';
					var param = {
								'upTransferTimeNodeVo':{
									'upTransferTimeNode':{
					...
					Ext.Msg.wait('保存中，请稍后...', '提示');
					    var returnSuc = function(json){
							...
							Ext.getCmp('queryUpTransferTimeNodeGridPaginId').moveFirst();
							win.hide();
						};
	...
	function imple(outFileName,flag){
		if(flag){
			...
			Ext.Ajax.request( {  
		       url : 'upTransferTimeNodeManageAction!implExcel.action',  
		       method : 'post',  
		       params : {  
		    	   'upTransferTimeNodeVo.filePath' : outFileName  
		          },  
		             ...
		             //隐藏进度条   
		              ...
		              if(responseArray.success){
		            	//addSize:增加条数,coverSize:覆盖条数,sumSize:总共条数,filePath:错误的信息的文件地址
			              msg="本次导入"+responseArray.upTransferTimeNodeVo.sumSize+"条数据，新增数据"+responseArray.upTransferTimeNodeVo.addSize+"条，覆盖原数据"+responseArray.upTransferTimeNodeVo.coverSize+"条。"	
							upTransferTimeNodeStore.reload();
		              }else{
	...
	function saveUpTransferTimeNode(){
		Ext.Msg.wait('保存中，请稍后...', '提示');
		//var msgWindow = null;
		var form = addForm.getForm();
		var formUrl;
		...
		switch (operateFlag) {
		case 1:
			formUrl = 'upTransferTimeNodeManageAction!addUpTransferTimeNode.action';
			param = {
					'upTransferTimeNodeVo':{
						'upTransferTimeNode':{
			...
			break;
		case 2:
			formUrl = 'upTransferTimeNodeManageAction!modifyUpTransferTimeNode.action';
			param = {
					'upTransferTimeNodeVo':{
						'upTransferTimeNode':{
...
//添加pannel
	...
	var viewport = Ext.create('Ext.Viewport', {
		...
		items: [queryForm,upTransferTimeNodeGrid]
	});
