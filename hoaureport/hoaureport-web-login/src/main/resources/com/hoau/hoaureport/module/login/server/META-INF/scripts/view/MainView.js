...
//切换部门的窗口
Ext.define('Butterfly.main.CurrentDeptChangeWindow', {
	...
	doChangeCurrentDept: function(deptCode){
        ...
        oms.requestJsonAjax('../login/loginAction!changeCurrentDept.action', params, successFun, failureFun);
	},
	//部门列表
	deptGridPanel : null,
...
//导航菜单树
Ext.define('Butterfly.main.MainNav',{
			...
			this.queryButton = Ext.create('Ext.button.Button',{ 
		    	...
		    	handler: function(){
		    		...
		    		if(!Ext.isEmpty(queryValue)){
		    	        ...
		    	        //Ajax请求得到所有查询到的节点全路径
		    	        oms.requestJsonAjax('../login/menuAction!queryTreePathForName.action', params, successFun, failureFun);
		    		}
		    	}
		    });
		}
		...
		mainNav.listeners = {
			...
			itemclick: function(node, record,item,index,e,eOpts){
				...
				if(!record.get('leaf')){
                    ...
                    try {
                        // 为叶子菜单记录访问事件到 Piwik
                        PiwikTracker.record(record.data.text + '(' + record.data.id + ')',
                            record.data.uri,
                            PiwikTracker.EVENT.MENU_CLICK);
                    }catch (e){
...
Ext.onReady(function(){
	...
	var treePanel = Ext.create('Butterfly.main.MainNav');
	var topPanel = Ext.create('Butterfly.main.topPanel');
...
function mapHomePage(){
	...
	Ext.Ajax.request({
		url:'loginAction!userHomeMapPage.action',
		method: 'POST',
		success: function(response) {
			var result = Ext.decode(response.responseText);
			if(result.success){
				var mapuri=result.homePageUri;
				var mapid=result.homePageId;
				var tabs = Ext.getCmp('tabPanel');
				if (tabs) {
					var tab = tabs.getComponent(mapid);
					//判断是否已经打开该面板
					if (!tab) {
						tab = tabs.add({
							'id': mapid,
							...
							closable:false,
							border:false,
							html: '<iframe id="'+mapid+'" src="'+mapuri+'" style="height:100%;width:100%;padding:0px;margin:0px;" frameborder="0"></iframe>'
						});
					}
				    tabs.setActiveTab(tab);
				}
