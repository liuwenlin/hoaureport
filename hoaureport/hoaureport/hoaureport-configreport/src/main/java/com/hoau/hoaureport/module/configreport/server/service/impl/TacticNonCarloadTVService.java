...
import com.hoau.hoaureport.module.configreport.server.dao.TacticNonCarloadTvMapper;
...
@Service
public class TacticNonCarloadTVService implements ITacticNonCarloadTVService{

	private static final Logger log = LoggerFactory.getLogger(TacticNonCarloadTVService.class);
	@Resource
	TacticNonCarloadTvMapper mapper;
	@Override
	public List<TacticNonCarloadTv> queryInfo(TacticNonCarloadTv param,
			int start, int limit) {
	...
	@Override
	public Long queryCount(TacticNonCarloadTv param) {
		return mapper.queryCountByCondition(param);
	}

	@Transactional
	@Override
	public void addInfo(TacticNonCarloadTv info) {
	...
	@Override
	public void updateInfo(TacticNonCarloadTv info) {
		UserEntity currUser = (UserEntity) UserContext.getCurrentUser();
		//工号
		String currUserCode = currUser.getEmpCode().substring(2);
		info.setModifyTime(new Date());
	...
	@Override
	public boolean isExist(TacticNonCarloadTv param) {
		TacticNonCarloadTv exitCondition = new TacticNonCarloadTv();
		...
		List<TacticNonCarloadTv> SalesLineInfos = queryInfo(exitCondition,0,10);
	...
	public Map<String, Object> bathImplPlatformAreaInfo(String path)
		...
		//导入结果
				...
				//把键值对 转换成 对象集合
				List<TacticNonCarloadTv> lines = new ArrayList<TacticNonCarloadTv>();
				  if(list.size() > 0){
				...
				for (Map<String, String> map : list) {
					TacticNonCarloadTv bean = new TacticNonCarloadTv();
					try {
				...
				for (int i = 0; i < lines.size(); i++) {
					TacticNonCarloadTv info = lines.get(i);
					try {
						if (info == null) {// 有异常
							continue;
						} else {
	...
	@Override
	public void addOrUpdateInfo(TacticNonCarloadTv info,
