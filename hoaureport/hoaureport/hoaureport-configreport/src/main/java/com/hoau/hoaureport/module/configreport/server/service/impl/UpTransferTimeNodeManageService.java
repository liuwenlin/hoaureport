...
import com.hoau.hoaureport.module.configreport.server.dao.UpTransferTimeNodeMapper;
...
/**
 * 上转移时间节点管理服务实现类
 * ClassName: UpTransferTimeNodeManageService 
 * @author 文洁
 * @date 2016年10月31
 * @version V1.0
 */
@Service
public class UpTransferTimeNodeManageService implements IUpTransferTimeNodeManageService{

	private static final Logger log = LoggerFactory.getLogger(UpTransferTimeNode.class);
	@Resource
	UpTransferTimeNodeMapper upTransferTimeNodeMapper;
	 ...
	 */
	@Override
	public List<UpTransferTimeNode> queryUpTransferTimeNode(UpTransferTimeNode param,
			int start, int limit) {
		RowBounds rowBounds = new RowBounds(start,limit);
		return upTransferTimeNodeMapper.queryUpTransferTimeNodeByCondition(param, rowBounds);
	}
	 ...
	 */
	@Override
	public Long queryUpTransferTimeNodeCount(UpTransferTimeNode param) {
		return upTransferTimeNodeMapper.queryUpTransferTimeNodeCountByCondition(param);
	}
	...
	@Transactional
	@Override
	public void addUpTransferTimeNode(UpTransferTimeNode record) {
		...
		upTransferTimeNodeMapper.insertSelective(record);
	}
	...
	@Transactional
	@Override
	public void updateUpTransferTimeNode(UpTransferTimeNode record) {
		upTransferTimeNodeMapper.updateByPrimaryKeySelective(record);
	}
	...
	@Transactional
	@Override
	public void repealAndAddUpTransferTimeNode(UpTransferTimeNode param) {
		//原纪录作废，并取得修改者编号
		String modifier = repealUpTransferTimeNode(param);
	 ...
	 */
	@Override
	public String repealUpTransferTimeNode(UpTransferTimeNode param) {
		UserEntity currUser = (UserEntity) UserContext.getCurrentUser();
		//工号
		String currUserCode = currUser.getEmpCode().substring(2);
		UpTransferTimeNode oldInfo = new UpTransferTimeNode();
		...
		updateUpTransferTimeNode(oldInfo);
		return currUserCode;
	}
	 ...
	 */
	@Override
	public boolean isExist(UpTransferTimeNode param) {
		//设置判断为已存在的条件
		UpTransferTimeNode exitCondition = new UpTransferTimeNode();
		...
		List<UpTransferTimeNode> upTransferTimeNodes = this.queryUpTransferTimeNode(exitCondition,0,10);
		return upTransferTimeNodes.size() > 0;
	}
	...
	//把键值对 转换成 对象集合
	List<UpTransferTimeNode> stationAreaInfos = new ArrayList<UpTransferTimeNode>();
	...
	for (Map<String, String> map : list) {
		UpTransferTimeNode bean = new UpTransferTimeNode();
		try {
			//大区
	...
	for (int i = 0; i < stationAreaInfos.size(); i++) {
		UpTransferTimeNode info = stationAreaInfos.get(i);
		try {
			if (info == null) {// 有异常
				continue;
			} else {
				...
				this.addOrUpdateUpTransferTimeNode(info, countMap);
			}
		} catch (Exception e) {
	 ...
	 */
	@Override
	public void addOrUpdateUpTransferTimeNode(UpTransferTimeNode info,
		...
		//查询历史有效信息
		UpTransferTimeNode queryParam = new UpTransferTimeNode();
		...
		List<UpTransferTimeNode> infoList = this.queryUpTransferTimeNode(queryParam, 0, 1);
		if(infoList.size() > 0){//已存在 就直接更新
			//更新数据
			info.setuId(infoList.get(0).getuId());
			this.repealAndAddUpTransferTimeNode(info);
			//覆盖+1
			//coverSize++;
		}else{
			//插入数据
			info.setImportTime(new Date());
			this.addUpTransferTimeNode(info);
		}
		//新增+1
		addSize++;
