/**主界面->部门负责人*/
Ext.define('Ty.Report.TacticNonCarloadTvAction.TacticNonCarloadTvActionModel', {
...
/***主界面->次日送达率store*/
Ext.define('Ty.Report.TacticNonCarloadTvAction.TacticNonCarloadTvActionStore', {
	extend : 'Ext.data.Store',
	model : 'Ty.Report.TacticNonCarloadTvAction.TacticNonCarloadTvActionModel',
	pageSize : 100,// 分页条数
	proxy : {
		type : 'ajax',
		url : 'tacticNonCarloadTvAction!queryInfo.action',
		actionMethods : 'POST',
		reader : {
			type : 'json',
	...
	listeners : {
		...
		beforeload : function(store, operation, eOpts) {
				var params = {
						'vo.entity.times':Ext.String.trim(Ext.getCmp('times').getValue()),
...
Ext.onReady(function(){
	...
	*/
	...
	//主界面->表格->store
		var managerStore=Ext.create('Ty.Report.TacticNonCarloadTvAction.TacticNonCarloadTvActionStore',{
			id:'managerStoreId'
		});
		...
		var  managerGrid =Ext.create('Ext.grid.Panel', {
	        ...
	        listeners : {
			 ...
			 dockedItems:[{//分页
		 			xtype:'pagingtoolbar',
		 			id: 'queryTacticNonCarloadTvActionGridPaginId',
		 			store:managerStore,
		 			dock:'bottom',
		 			displayInfo : true,
		 			autoScroll : true
...
function imple(outFileName,flag){
			if(flag){
				...
				Ext.Ajax.request( {  
			       url : 'tacticNonCarloadTvAction!implExcel.action',  
			       method : 'post',  
			       params : {  
